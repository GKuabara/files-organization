...
static void _vh_global_init(struct _v_files *files) {
    ...
    g_off_next_reg_t off_next_reg = V_HEADER_SIZE;
    fwrite(&off_next_reg, sizeof(g_off_next_reg_t), 1, files->bin);

    g_amnt_reg_t n_reg = 0;
    fwrite(&n_reg, sizeof(g_amnt_reg_t), 1, files->bin);

    g_amnt_rem_reg_t amnt_removed_reg = 0;
    fwrite(&amnt_removed_reg, sizeof(g_amnt_rem_reg_t), 1, files->bin);
}
...
static struct _v_reg_update *vdr_write_new_reg(FILE *bin, g_off_next_reg_t offset, string *tokens) {
...
static g_off_next_reg_t _vh_update(FILE *bin, g_status_t stats, struct _v_reg_update *update, g_off_next_reg_t l_offset, int amnt_reg, int amnt_rmv) {
    ...
    fwrite(&amnt_rmv, sizeof(g_amnt_rem_reg_t), 1, bin);

    return next_reg;
}

static void _vdr_insert_regs_from_csv(struct _v_files *files) {
    g_off_next_reg_t offset = V_HEADER_SIZE; 
    int amnt_reg = 0;
    int amnt_rmv = 0;
